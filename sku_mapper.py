...
def index():
    try:
        ...
        for competitor in mapping_tables.keys():
            c.execute(
                f'''CREATE TABLE IF NOT EXISTS {competitor}_matched_products (mecp_sku text, competitor_sku text)''')
        ...
        for mecp_sku in mecp_skus:
            for competitor, mapping_table in mapping_tables.items():
                ...
                if pd.isna(competitor_sku):
                    ...
                    c.execute(f"INSERT INTO {competitor}_matched_products VALUES (?, ?)",
                              (mecp_sku, str(competitor_sku)))
...
def add_competitor():
    if request.method == 'POST':
        ...
        c = mecp_conn.cursor()
        c.execute(
            f'''CREATE TABLE IF NOT EXISTS {competitor_name}_matched_products (mecp_sku text, {competitor_name}_sku text)''')
        mecp_conn.commit()
        mecp_conn.close()
        return redirect(url_for('index'))
    else:
